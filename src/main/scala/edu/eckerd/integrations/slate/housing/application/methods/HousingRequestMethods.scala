...
trait HousingRequestMethods {
  ...
  def generateTermCode(housingRequest: HousingRequest):Xor[String,String] = {
  ...
  def UpdateDatabase(housingRequest: HousingRequest): Future[Xor[String, Int]] = {
    val f = for {
  ...
  def xorFutureConverter[A, B](xor: Xor[B, Future[A]]): Future[Xor[B,A]] = {
    xor.bimap((a) => Future.successful(a), (b) => b).bisequence
  }

  def futureXorFutureConverter[A,B](future: Future[Xor[B, Future[A]]]): Future[Xor[B, A]] = {
    for {
      xor <- future
